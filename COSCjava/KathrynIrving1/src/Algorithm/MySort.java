...
public class MySort {
    ...
    public static MyArrayList bubbleSort(MyArrayList pal) {
		...
		Comparable<Comparable> second;
        
        for (i = 1; i < n; i++) 
        {
            for (j = n - 1; j >= i; j--) 
            {
                first = (Comparable<Comparable<Comparable>>) pal.elementAt(j - 1);
                second = (Comparable<Comparable>) pal.elementAt(j);
    ...
    public static MyArrayList selectionSort(MyArrayList pal) {
        ...
        Comparable<Comparable> smallest, current;
        
        for (i = 0; i < n - 1; i++) 
        {
            smallPos = i;
            smallest = (Comparable<Comparable>) pal.elementAt(smallPos);
            for (j = i + 1; j < n; j++) 
            {
                current = (Comparable<Comparable>) pal.elementAt(j);
                if (current.compareTo(smallest) < 0) 
                {
                    smallPos = j;
                    smallest = (Comparable<Comparable>) pal.elementAt(smallPos); //= current
                }
            }
            if (smallPos != i) 
            {
    ...
    public static void insertionSort(MyArrayList pal, int left, int right) {
        ...
        for (outer = left + 1; outer <= right; outer++) 
        {
            ...
            inner = outer;
            while (inner > left && ((Comparable<Object>) pal.elementAt(inner - 1)).compareTo(temp) > 0) 
            {
    ...
    public static void shellSort(MyArrayList pal) {
        ...
        while (h > 0) 
        {
            for (outer = h; outer < pal.size(); outer++) 
            {
                ...
                inner = outer;
                while (inner > h - 1 && ((Comparable<Object>) pal.elementAt(inner - h)).compareTo(target) > 0) 
                {
	...
	public static void medianOf3(MyArrayList pal, int left, int right) {
        ...
        if (((Comparable<Object>) pal.elementAt(left)).compareTo(pal.elementAt(mid)) > 0) 
        {
            swap(pal, left, mid);
        }
        if (((Comparable<Object>) pal.elementAt(mid)).compareTo(pal.elementAt(right)) > 0) 
        {
            swap(pal, mid, right);
        }
        if (((Comparable<Object>) pal.elementAt(left)).compareTo(pal.elementAt(mid)) > 0) 
	...
	public static int partition(MyArrayList pal, int left, int right) {
        ...
        while (true) 
        {
            while (((Comparable<Object>) pal.elementAt(++left)).compareTo(pivot) < 0);
            while (((Comparable<Object>) pal.elementAt(--right)).compareTo(pivot) >= 0);
            if (left > right) 
            {
                break;
            } 
